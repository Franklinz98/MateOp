...
List<Exercise> generateRandomExercise(
  ...
  for (var i = 1; i <= numberOfExercises; i++) {
    var operator1 =
        minNumber + Random().nextInt(maxNumber - minNumber) as double;
    var operator2 =
        minNumber + Random().nextInt(maxNumber - minNumber) as double;
    myExercises.add(
      Exercise(
...
Future<List<Exercise>> generateExercisesFromLOPerformance(
  ...
  List<Exercise> myExercises = List<Exercise>();
  Intensity intensitiy = Intensity();
  double totalPerformance = getTotalPerformance(myPerformance);
  ...
  for (var i = 1; i < myPerformance.getNumberOfLo(); i++) {
    int numberExercPerLo =
        (numberOfExercises * myPerformance.loIntensities["LOIN$i"]) as int;
  ...
  for (var i = 0; i < myPerformance.getNumberOfLo(); i++) {
    int numberOfDF =
        (numberPerLo[i] * myPerformance.loIntensities["LOINDF$i"]) as int;
    int numberOfDD =
        (numberPerLo[i] * myPerformance.loIntensities["LOINDD$i"]) as int;
...
Future<List<Exercise>> _generateExercisesPerLO(int numberExerc,
  ...
  for (var i = 0; i < numberExerc; i++) {
    ...
    Exercise oneExc = Exercise(
      loID: loId,
      firstOperator: sumOp1 as double,
      secondOperator: sumOp2 as double,
      answer: calculateAnswer(sumOp1 as double, sumOp2 as double, opType),
      type: type,
    );
